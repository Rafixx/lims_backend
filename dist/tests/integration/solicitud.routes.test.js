...
const createDataContext = async () => {
    ...
    const cliente = await (0, supertest_1.default)(app_1.default).post('/api/clientes').send({
        nombre: 'Cliente Test',
        nif: '12345678A',
        direccion: 'Dirección Test',
    });
    //crear previamente una prueba válida
    const prueba = await (0, supertest_1.default)(app_1.default).post('/api/pruebas').send({
        cod_prueba: 'Prueba Test',
        prueba: 'Descripción de prueba',
    });
    // Crear previamente una solicitud válida
    const solicitud = await (0, supertest_1.default)(app_1.default).post('/api/solicitudes').send({
...
describe('Solicitud Routes - Integration', () => {
    ...
    describe('GET /api/solicitudes', () => {
        it('debería devolver la lista de solicitudes con relaciones', async () => {
            ...
            const response = await (0, supertest_1.default)(app_1.default).get('/api/solicitudes');
